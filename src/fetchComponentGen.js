...
const fetchComponentGen = (doc) => {
   let querylist = _obj.querySelectorAll(doc, `[${_pam.gb_attrname}*=".html"],[${_pam.gb_attrname}*=".htm"]`)
   ...
   if (i) {
      ...
      for (let i = 0, l = _obj.length(arrkey); i < l; i++) {
         if (_obj.indexOf(arrkey[i], '-') > -1) {
            const name = arrkey[i];
            fetch(_obj.getAttribute(domjson[name], _pam.gb_attrname)).then(r => {
               if (r.status !== 200) {
                  return _pam.gb_null;
               }
               ...
               domtemp.innerHTML =
                  _obj.replace(res.txt, _reg.reg10, (s, s1) => {
                     ...
                     if (_pam.gb_parseto_template.has(s1)) {
                        if (_obj.indexOf(s, '</') === 0) {
                           return `</${_pam.gb_template}>`;
                        }
                        s = _obj.trim(_obj.substring(s, _obj.length(s1) + 1));
                        return `<${_pam.gb_template}\x20-='${s1}'\x20${s}`;
                     } else if (s1 === _pam.gb_template && _obj.indexOf(s, '</') !== 0) {
